...
product_sales_metrics as (
    select
        ...
        case 
            ...
            then (coalesce(sum(case when s.has_discount = 1 then 1 else 0 end), 0) / sum(case when s.sales_order_id is not null then 1 else 0 end)) * 100
            else 0
    ...
    group by 
        p.product_id, p.category_name, p.subcategory_name, p.color, p.p_size, p.weight_category, p.price_category, p.sourcing_type, p.sellability_status, p.product_status, p.manufacturing_speed, p.standard_cost, p.list_price, p.profit_margin_percentage, p.profit_per_unit
),
